...
| `--log-format`      |       | string | Log format: `text` (a terminal) or `json` (a container) (env `CF2CNP_LOG_FORMAT`) | `text`  |
| `--log-level`       |       | string | Log level: `debug`, `info`, `warn`, `error` (env `CF2CNP_LOG_LEVEL`). `/health` is always debug | `info`  |
...
with a short snake_case `reason` (and never the token, the `Authorization` header, cookies, or the request body).

Example (json, three lines of one empty POST /generate):

```json
{"time":"2026-09-16T08:00:00.000Z","level":"INFO","msg":"listening","addr":":8080","version":"v0.8.0","log_format":"json","log_level":"info","auth_enabled":false,"allowed_origins":"*","dns_profile":"auto","dns_resolver":""}
{"time":"2026-09-16T08:00:01.000Z","level":"WARN","msg":"refused","request_id":"abc-123","status":400,"reason":"body_empty"}
{"time":"2026-09-16T08:00:01.001Z","level":"WARN","msg":"request","request_id":"abc-123","method":"POST","path":"/generate","status":400,"bytes":58,"duration_ms":1.2,"client":"10.0.0.1","user_agent":"curl/8.7.1"}
```
...
| `GET`  | `/download/{id}` | Download a policy generated by a JSON-mode request (cached for 10 minutes; `{id}` is the flow's UUID for a single flow) | _none_ | CiliumNetworkPolicy YAML as an attachment; 410 when the policy expired (kept 10 minutes), 404 when nothing was generated under the id |
